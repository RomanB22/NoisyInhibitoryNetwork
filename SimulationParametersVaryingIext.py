...
ChosenModel = 5 #Models = {0:'IzhiTypeI', 1:'IzhiTypeII', 2:'LIFModel', 3:'AdExModel', 4:'GuillemModel'}
...
transientTime = 900*b2.ms#1500*b2.ms #900#Time until the network reaches the stationary state
simulationEnd = 1000*b2.ms#1800*b2.ms #1200#Time of simulation end
PlotingTime = 100 #Time which is going to be plotted (in ms). Between 200 and 400 ms is okay. The interval goes between [simulationEnd-PlotingTime, simulationEnd]
SavingTime = 100*b2.ms #Time interval of the variables measured in the simulation to save. Be careful to use so much time here, because the size of all the generated .csv files could be huge. Maximum around 400 ms is okay. Interval is [simulationEnd-SavingTime, simulationEnd]
Neurons2Save = 2 #How many neurons to fully follow across the simulation time. Same as before, too much neurons creates huge .csv files
ChosenNeuronModelGuillem = 57 # MAximum membrane time constant, equal to 6.96 ms

sigmaAux =[8.7610]# [0.1334,0.1884,0.5110,1.2589,3.7610,5.7610,7.2610,9.7610]#np.sort(np.concatenate((np.around(np.logspace(-2,1,41,endpoint=True),4),np.linspace(0.011,10.011,41, endpoint=True))))

JAux = [113]#np.sort(np.concatenate((np.around(np.logspace(0,2,21,endpoint=True),1),np.linspace(3,153,31, endpoint=True))))

#I0Aux = np.linspace(-8,55,64, endpoint=True)

...
VariableDrive = False
ConductanceBased = False; Esyn = -50.*b2.mV; #shunting -55 mV, hyperpolarizing = -75 mV

alphaAux = [1]# Scaling for the synaptic times
NAux = [3000]#[800,1000,1400,1800,2200,3000]
Nu0Aux = [30.]
ConnectionProbAux = [1]
InCond = ['random',1]#['cluster',1] #'random' or 'cluster' and second element is number of clusters
...
transientTime = 1500*b2.ms#1500*b2.ms #900#Time until the network reaches the stationary state
simulationEnd = 1800*b2.ms#1800*b2.ms #1200#Time of simulation end
...
JAux = [1.]#np.sort(np.concatenate((np.around(np.logspace(0,2,21,endpoint=True),1),np.linspace(3,153,31, endpoint=True))))

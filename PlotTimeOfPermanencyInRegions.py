...
FileSynchronyIndex = "MapSupraSNCurrent3000_"+str(nu0)+"_CSV_"+ModelDescriptor+".csv"
#MapSubSNPCurrent3000_30_CSV_IzhiTyII.csv
#MapBistCurrent3000_30_CSV_IzhiTyII.csv
#MapSupraHopfCurrent3000_30_CSV_IzhiTyII.csv
#MapSupraSNCurrent3000_30_CSV_IzhiTyII.csv
...
if ModelDescriptor == "IzhiTyI":
	levels = [min(Z.flatten()),0.129321*10/0.09,max(Z.flatten())]
elif ModelDescriptor == "IzhiTyII":
	levels = [min(Z.flatten()),0.1795*10.,0.2625*10.,0.4225*10.,max(Z.flatten())]



fig = plt.figure()
ax = fig.gca()
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=30, cmap=cm.viridis)
...
plt.savefig("SupraSN{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))

...
execfile("SimulationParametersMainAlternative2.py")
...
#IndexJ = np.argwhere(abs(Matrix[:,1]-J/b2.mV)<1e-1)
#IndexSigma = np.argwhere(abs(Matrix[:,2]-sigma/b2.mV)<5e-1)
#Indexes = np.intersect1d(IndexJ,IndexSigma) #Intersection between two arrays
#Iext_Guess = min(Matrix[Indexes,0])*IextUnits
...
if FirstBisection:
	if ChosenModel==4: #Via model
		...
		Iext_Guess = 12.4*IextUnits
		I0_solution = bisectionSearch(I0_init=Iext_Guess-4*abs(Iext_Guess),I0_end=Iext_Guess+4*abs(Iext_Guess),Nu_0=Nu_0, DeltaNu_0=DeltaNu_0,I0_lowbound=-5.*abs(Iext_Guess),I0_uppbound=10.*abs(Iext_Guess),I0Units=IextUnits,transientTime=transientTime)/IextUnits
	...
	elif ChosenModel==0: #Izhikevich Type 2 resonator model

...
MaxCurrent = np.loadtxt(FileMaxInput,delimiter=",",skiprows=1,usecols=cols)
MeanCurrent = np.loadtxt(FileMeanInput,delimiter=",",skiprows=1,usecols=cols)
...
MaskSigma = np.zeros(np.shape(MaxCurrent))
...
if ModelDescriptor == "IzhiTyI":
	levels = np.sort([min(Z.flatten()),0.129321*10/0.09,max(Z.flatten())])
elif ModelDescriptor == "IzhiTyII":
	levels = np.sort([min(Z.flatten()),0.1795*10.,0.2625*10.,0.4225*10.,max(Z.flatten())])
elif ModelDescriptor == "AdExTyIIAH":
	factor=10./np.array(CAdEx)
	levels = np.sort([min(Z.flatten()),factor*165,factor*184.8613,factor*185.2901,max(Z.flatten())])
	print(levels)
elif ModelDescriptor == "LIF":
        levels = [min(Z.flatten()),20.,max(Z.flatten())]

fig = plt.figure()
ax = fig.gca()
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=levels, colors=['r','g','b','y'])
...
plt.savefig("MaxInput{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))

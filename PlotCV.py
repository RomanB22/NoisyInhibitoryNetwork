...
FileSynchronyIndex = "CV{N}_{nu}_CSV_{Folder}.csv".format(N=NAux[0],nu=nu0,Folder=ModelDescriptor)
...
ax = fig.gca()
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=np.linspace(0,0.8,41,endpoint=True), cmap=cm.viridis)
...
plt.savefig("CVMap{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))

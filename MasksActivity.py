...
nu0=30
ModelDescriptor = "AdExTyIISN"


cols=np.arange(1,53)

FileSynchronyIndex = "ActivityMask_"+str(nu0)+"_CSV_"+ModelDescriptor+".csv"
...
ax = fig.gca()
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=[0,0.5,1], cmap=cm.viridis)

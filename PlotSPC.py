...
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=np.linspace(0.,1.,80,endpoint=True), cmap=cm.viridis)
...
plt.savefig("SPCMap{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))

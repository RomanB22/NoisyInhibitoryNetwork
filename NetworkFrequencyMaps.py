...
nu0=30
ModelDescriptor = "IzhiTyII"

cols=np.arange(1,53)
NAux=[3000]

FileSynchronyIndex = "NetwFreqMap"+str(nu0)+"CSV_"+ModelDescriptor+"_N_"+str(NAux[0])+".csv"
...
ax = fig.gca()
cont = ax.contourf(np.log(X), np.log(Y), np.transpose(Z), levels=[27,31,200], cmap=cm.viridis)
...
plt.savefig("NetwFreq{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))

...
def CalculSynchInd(VoltageMon,fullResult=False):
	...
	PopulationSquaredStdDev = np.mean(PopulationMeanVoltageAcrossTime**2) - np.mean(PopulationMeanVoltageAcrossTime)**2
	NeuronSquaredStdDev = np.mean(MembraneVoltageWholePopulation**2,axis=0) - np.mean(MembraneVoltageWholePopulation,axis=0)**2
	MeanNeuronSquaredStdDev = np.mean(NeuronSquaredStdDev)
...
def CalculBinderCumul(VoltageMon):
	...
	return BinderCumul
##################################################################################
def CreatePlot2DModel(simulationEnd,ISI_times,SpikeMon,VarMon,PlotFile,NumbNeurons, Nu_0=30,IntervalLong=200,NeuronIndex=0,v_th=20*b2.mV,bin_lim = None, num_bins = None, Theta=False,ChosenModel=None):
	"""
	...
	plt.subplot(211)
	Npoints = int((xlim_right-xlim_left)/1) # Timestep 1 ms 
	...
	if Theta:
		ax4b = ax4.twinx()
		ax4b.plot(VarMon.t/b2.ms,VarMon.I0[0]/IextUnits,'-c')
		ax4b.set_ylabel('Theta Input (mV)', color='c')
		#ax4b.set_ylim(-1.*max(VarMon.I0[0]/IextUnits),1.*max(VarMon.I0[0]/IextUnits))
		ax4b.set_ylim(2.*min(VarMon.I0[0]/IextUnits)-0.1,2.*max(VarMon.I0[0]/IextUnits)+0.1)		
...
"""
	...
	if ChosenModel==0: # IzhiTypeI
		...
		if ChosenSubmodelAdEx==2: # AdEx AndronovHopf
			ax3.plot(VarMon.t/b2.ms, factor*164.9*np.ones(np.shape(VarMon.t/b2.ms)), 'r--')#(-233.1562) is the analytical value
	...
	ax3.plot(VarMon.t/b2.ms, VarMon.Iext[0]/IextUnits + VarMon.RecurrentInput[NeuronIndex].T/b2.mV, 'g')
	ax3.plot(VarMon.t/b2.ms, VarMon.Iext[0]/IextUnits + np.mean(VarMon.RecurrentInput[NeuronIndex].T/b2.mV) *np.ones(np.shape(VarMon.t/b2.ms)), 'b')

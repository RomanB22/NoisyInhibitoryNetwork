...
DoubletsMode_
DoubletsProportion_
DoubletsStandardDeviation_
"""
FileSynchronyIndex = FileName+str(nu0)+"_CSV_"+ModelDescriptor+".csv"
...
Z = ChiInfinite*factor
#Z = SynchronousPart*factor

levels = np.linspace(minimumOfScale,9.26,101,endpoint=True)
...
ax.set_xlim(np.log(min(sigma)), np.log(max(sigma)))
ax.set_ylim(np.log(min(J)), np.log(max(J)))

plt.xlim((np.log(0.1),np.log(max(sigma))))
...
plt.savefig(FileName+"{N}_{nu}_{Folder}.svg".format(N=3000,nu=nu0,Folder=ModelDescriptor))
